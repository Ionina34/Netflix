...
function search_staff_by_parameters_sort(data_value, alphabet_value, created_value, startPage) {
	$.ajax({
		type: "GET",
		url: "http://localhost:8000/admin/screenwriters/sort",
		data: {
			data: data_value,
...
function deleteScreenwriter(screenwriterId){
	data={}
	data["id"]=screenwriterId
	
	$.ajax({
		type: "DELETE",
		url: "http://localhost:8000/admin/screenwriters/delete/"+screenwriterId,
...
function viewScreenwriters(response) {
	...
	let row = '<table style="border: 1px solid rgb(0, 0, 0);">' +
		...
		'</tr></thead><tbody>';
	$.each(response.content, function(i, screenwriter) {
		let rowScreenwriter= '<tr id="'+screenwriter.id+'" >' +
			'<td style="border: 1px solid rgb(0, 0, 0);">' + index + '</td>' +
			'<td style="border: 1px solid rgb(0, 0, 0);">' + screenwriter.name + '</td>' +
			'<td style="border: 1px solid rgb(0, 0, 0);">' + screenwriter.brief_biography + '</td>' +
			'<td style="border: 1px solid rgb(0, 0, 0);">' + moment(screenwriter.birthday).format('DD-MM-YYYY') + '</td>' +
			'<td style="border: 1px solid rgb(0, 0, 0);"><button class="btn btn-warning" '+
			'><a href="/admin/screenwriters/update/'+ screenwriter.id +'">Редактировать</a></button></td>' +
			'<td style="border: 1px solid rgb(0, 0, 0);"><button class="btn btn-danger" onclick="deleteScreenwriter('+screenwriter.id+')">Удалить</button></td>' +
			'</tr>';
		row += rowScreenwriter;
		index++;
	});
...
$(document).ready(function() {
	function fetch_screenwriters(startPage) {
		$.ajax({
			type: "GET",
			url: "http://localhost:8000/admin/screenwriters/get",
			data: {
				page: startPage,
				size: 12
			},
	...
	$(document).on("click", "ul.pagination li a", function() {
		...
		if (val.toUpperCase() === "« FIRST") {
			...
			if (page_sort)
				...
				fetch_screenwriters(totalPages - 1);
			$("li.active").removeClass("active");
			// add .active to next-pagination li
			currentActive.next().addClass("active");
